...
class Quadmaps:
    ...
    def load_mini_map(self):
        for l1 in range(self.current_box[0]-1,self.current_box[0]+2):
            for l2 in range(self.current_box[1]-1,self.current_box[1]+2):
    ...
    def focus_on_position(self):
        focused_map = [[0.5]*(self.array_length-5) for _ in range(self.array_length-5)]
        cnt_y=0
        cnt_x=0
        for l1 in range(self.current_grid_position[0]-(int(self.array_length/2)-3),self.current_grid_position[0]+int(self.array_length/2)-2):
            for l2 in range(self.current_grid_position[1]-(int(self.array_length/2)-3),self.current_grid_position[1]+int(self.array_length/2)-2):
    ...
    def scan(self,ranges):
        theta_strips=1
        laser_range=3.5
        round_off_to_decimal_spaces = 1
        for cnt,i in enumerate(ranges):
            angle=math.radians(theta_strips*cnt)+self.current_orientation
            if (angle <= (-180)):
                angle+=360
            elif (angle>180):
                angle-=360
            ...
            distance_to_travel_to_mark_black= 0.1 / larger
            if i <=laser_range:
                j=0.000
                while(j<float(i)-(0.1*distance_to_travel_to_mark_black)):
                    x=int(round(float(j)*math.cos(angle),round_off_to_decimal_spaces)*math.pow(10,round_off_to_decimal_spaces))
                    x=min(x,int(laser_range*math.pow(10,round_off_to_decimal_spaces)))
                    x=max(-int(laser_range*math.pow(10,round_off_to_decimal_spaces)),x)
                    y=int(round(float(j)*math.sin(angle),round_off_to_decimal_spaces)*math.pow(10,round_off_to_decimal_spaces))
                    y=min(y,int(laser_range*math.pow(10,round_off_to_decimal_spaces)))
                    y=max(-int(laser_range*math.pow(10,round_off_to_decimal_spaces)),y)
                    x+=self.current_grid_position[1]
                    y=self.current_grid_position[0]-y  
                    
                    self.mini_map[y][x]=max(0.1,(1/(1+(((1-0.3)/0.3)*((1-self.mini_map[y][x])/self.mini_map[y][x])))))
                    j+=distance_to_travel_to_mark_black
                
                x=int(round(float(i)*math.cos(angle),round_off_to_decimal_spaces)*math.pow(10,round_off_to_decimal_spaces))
                y=int(round(float(i)*math.sin(angle),round_off_to_decimal_spaces)*math.pow(10,round_off_to_decimal_spaces))
                x+=self.current_grid_position[1]
                y=self.current_grid_position[0]-y   
    ...
    def split_map_to_boxes(self):
        control_loop_x = self.current_box[1]-1
        control_loop_y = self.current_box[0]-1
